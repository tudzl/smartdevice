...
# version 3.5 add bh1750  2019.8.5
# version 3,4 test
...
title = M5Title(title="  Smart Device ESP32 MQTT demo V3.5L", x=3 , fgcolor=0xff9900, bgcolor=0x1F1F1F)
...
try:   
           ...
           lux_data=BH.luminance(BH1750.ONCE_HIRES_1)
           BH1750_flag = True
...
try:
            ...
            while ENV_insert:
               if i2c.is_ready(IIC_Address_ENV):
                  ...
                  label3.setText("Sensor: OK!")
                  wait_ms(200) 
               if i2c.is_ready(BH1750_add):
                  ENV_insert =0
                  BH1750_flag=True
                  label3.setText("Sensor: Light!")
                  wait_ms(200) 
            pass
...
while True:
  ...
  if  MQTT_data_send_flag == 1:
      ...
      data_payload = data_payload+str("[")+str(T)+str(",")+str(H)+str(",")+str(A)+str(",")+str(run_cnt&0xFF)+str(",")+str(lux_data)+str("] ") 
      #data_payload = data_payload+str("[")+str(T)+str(",")+str(H)+str(",")+str(A)+str(",")+str(run_cnt&0xFF)+str(",0] ") 
