...
print(i2c0.read(2))
print(i2c0.read_u8(0x23))
i2c0.write_u8(0x00, 0x00)
print(i2c0.read_u16(0x00, byteorder="big"))
...
#IIC scan debug code by l.zhou not working
...
while True:
    ...
    #a.writeto(BH1750_add,0x20)
    try:   
           reset = 0x07
           off = 0x00
           #IIC_b=i2c_bus.easyI2C(i2c_bus.PORTA,BH1750_add)
           #IIC_b.write_u8(BH1750_add,0x20) # will send BH1750_add   BH1750_add 0x20 
           #IIC_b.write_u8(0x20,0) # will send BH1750_add    0x20 0x00
           #IIC_b.write_u8(0x20) # not working!
           #data = IIC_b.read(2)   # not working!
           #data = i2c1.read_u16(BH1750_add, byteorder='big')
           #a.write_u8(BH1750_add,0x20)
           #a.readfrom(BH1750_add, 2)  # need test
           #data = IIC_b.read(2) 
           #wait(0.12)
           a.writeto(BH1750_add, bytes([0x20]))        # send BH1750_add 0x20, working!
           #i2c.writeto(0x42, 'hello')          # write 5 bytes to slave with address 0x42
           ...
           #data=a.readfrom(BH1750_add, 2)
           #data = IIC_b.read_u16()
           #data = IIC_b.readfrom(BH1750_add, 2)   # 
           label_iic.setText("IIC read: "+"%x%%" % (data[0])+"%x%%" % (data[1]))  
