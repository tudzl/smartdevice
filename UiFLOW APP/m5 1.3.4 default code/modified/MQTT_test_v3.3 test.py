...
# Iot Eclipse:   iot.eclipse.org   1883   stsmd/+/+
...
def buttonA_wasPressed():
  ...
  m5mqtt.publish(str('test_ESP32_smartdevice'),str('M5 is online'))
...
def buttonB_wasPressed():
  ...
  if MQTT_data_send_flag ==1:
   ...
   m5mqtt.publish(str('stsmd/ESP32/info'), "Smart device ESP32_M5_Zell data:")
   wait_ms(100)
  else:
    m5mqtt.publish(str('test_ESP32_smartdevice'), "Smart device ESP32_M5_Zell Go OFF:")
    m5mqtt.publish(str('stsmd/ESP32/info'), "Smart device ESP32_M5_Zell Go OFF:")
...
while True:
  ...
  if  MQTT_data_send_flag == 1:
      ...
      m5mqtt.publish(str('test_ESP32_smartdevice'),"T.="+str(T)) #old
      m5mqtt.publish(str('stsmd/ESP32/Alert'),"T.="+str(T))
      wait_ms(200)
      label2.setText("MQTT_send:"+"A.P.="+str(A))
      m5mqtt.publish(str('test_ESP32_smartdevice'),"A.P.="+str(A))
      m5mqtt.publish(str('stsmd/ESP32/Alert'),"A.P.="+str(A))
      wait_ms(200)
      label2.setText("MQTT_send:"+"H.="+str(H))
      m5mqtt.publish(str('test_ESP32_smartdevice'),"H.="+str(H))
      m5mqtt.publish(str('stsmd/ESP32/Alert'),"H.="+str(H))
      wait_ms(200)
  ...
  if MQTT_Alert_send_flag ==1:
	  ...
	  #Alert_payload_str = "".join(map(chr,Alert_payload))
	  #Alert_msg
	  kk=[]
      ...
      m5mqtt.publish(str('stsmd/ESP32/Alert'),str(Alert_payload_str) )
      Msg_cnt_T =Msg_cnt_T+1
      label2.setText("Alert_sent: "+"Alert:0x"+"%x%%" % (Alert_payload) ) #show in hex, ok
      wait_ms(200)
