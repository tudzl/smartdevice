...
# version 3.2 M5stack_01_SH200Q working great bak 2019.8.01
...
def buttonA_wasPressed():
  ...
  m5mqtt.publish(str('stsmd/'+Device_ID+'/info'), "Smart device is online!")
...
def buttonB_wasPressed():
  ...
  if MQTT_data_send_flag ==1:
   ...
   wait_ms(100)
    ...
    m5mqtt.publish(str('stsmd/'+Device_ID+'/info'), "Smart device Go offline!")
...
title = M5Title(title="  Smart Device ESP32 MQTT demo V3.2", x=3 , fgcolor=0xff9900, bgcolor=0x1F1F1F)
...
while True:
  ...
  if MQTT_Alert_send_flag ==1:
      ...
      kk.pop()
      kk.pop()
      #kk.pop()  #only for test
      m5mqtt.publish(str('stsmd/'+Device_ID+'/alert'),str(Alert_payload_str) )
      Msg_cnt_T =Msg_cnt_T+1
      #label2.setText("Alert_sent: "+"Alert:0x"+"%x%%" % (Alert_msg) ) #show in hex, ok
      #label2.setText("Alert_sent: "+ str(kk )#only for test
      wait_ms(400)
