...
def buttonB_wasPressed():
  ...
  m5mqtt.publish(str('test_ESP32_smartdevice'),str('M5 MQTT is running'))
  wait(0.1)
  pass
...
def buttonC_wasPressed():
  ...
  if MQTT_Alert_send_flag ==1:
     ...
     label1.setText('Smartdevice stop sending alerts...')
  if LCD_EN <= 0:
     LCD_EN = 20
  wait(0.1)
  pass
...
label2 = M5TextBox(15, 80, "MQTT_send", lcd.FONT_DejaVu24,0xAFFFAF, rotate=0) #MQTT sent msg display
...
#--------MQTT----------------
#m5mqtt.subscribe(str('test_ESP32_smartdevice'), fun_test_m5stack_zell_)  #will echo received msg
m5mqtt.subscribe(str('test_ESP32_smartdevice_server'), fun_test_m5stack_zell_)
#wifiCfg.doConnect('TC', 'sthz@2020')
wait(0.1)
m5mqtt.start()
wait(0.1)
...
m5mqtt.publish(str('test_ESP32_smartdevice'),str('M5 is now online'))
...
while True:
  ...
  if Alarm_flag == 1:
    lcd.circle(220, 160, 4, lcd.RED, lcd.RED)  # draw Alarm_flag indication
    rgb.setColorAll(0xff0000)
    for i in range(10, 180, 10):
      rgb.setBrightness(i)
      #speaker.setVolume(1)
