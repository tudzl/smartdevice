...
#V2.0 improved , add min max set , works ok
#V1.0 works ok
from m5stack import *
from uiflow import *
from m5ui import *
...
def buttonA_wasPressed():
  # global params
  global Lum_value_tmp
  speaker.tone(446, 120, 1)
  Lum_value_tmp =BH.luminance(BH1750.ONCE_HIRES_1)
  label_Lum_value.setText("Lum.set: "+str("%.1f" % Lum_value_tmp)+"lux")  
  pass
...
time.sleep(0.6) 
setScreenColor(0x111111)
title = M5Title(title="  ESP32 IIC debug, BH1750 Lux", x=3 , fgcolor=0xEFEFDF, bgcolor=0x1F1F1F)
#IIC  init
...
label_max = M5TextBox(190, 140, "Lum.Max:", lcd.FONT_Default,0xFFCF1F, rotate=0)
label_min = M5TextBox(190, 160, "Lum.Min:", lcd.FONT_Default,0xAFEFDF, rotate=0)
label_Lum_value = M5TextBox(190, 180, "Lum.set:", lcd.FONT_Default,0xEFEFDF, rotate=0)
...
while True:
    ...
    lcd.print( "total addr: "+str(l),20, 170, 0xFFAAFF)
    ...
    try:   
           ...
           if data>Lum_value_max:
              Lum_value_max =data

           if data<Lum_value_min:
              Lum_value_min =data
    except:
           label_iic.setText("IIC read: N.A. ")  
           pass
    ...
    label_max.setText("Max: "+str("%.1f" % Lum_value_max)) 
    label_min.setText("Min: "+str("%.1f" % Lum_value_min)) 
    start = time.ticks_ms() # get millisecond counter   
