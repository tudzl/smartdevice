...
float ZFilter::Filter(float origin)
{

	
	/* Algorithm */
	X_cur  = origin;
	X_post = p*X_cur+p1*X_pre1+p2*X_pre2;
	//X_post = p*X_cur; //test only
	//--renew data
	X_pre2 = X_pre1;
	X_pre1 = X_post;
	//X_pre1 = X_post
...
float KalmanFilter::Gaussian_Noise_Cov(void)
{
	...
	//srand((int)analogRead(0)); //ori
	
	/* Get random number */
	for(int i=0; i<10; i++)	
	{
		//index = esp_random(100); //by zell
		//index = esp_random(); //by zell
		index = esp_random()%100; //ori
	    //index = (int)rand()%100; //ori
