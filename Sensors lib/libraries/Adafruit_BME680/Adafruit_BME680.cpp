...
bool Adafruit_BME680::endReading(void) {
  ...
  if (_gasEnabled) {
    ...
    gas_resistance = (uint32_t) NAN;   //    /** Gas resistor (ohms) assigned after calling performReading() or endReading() **/
    //NaN (Not-A-Number) value of type double. The NaN values are used to identify undefined or non-representable values for floating-point elements
  }
