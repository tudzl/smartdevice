...
/*!
    ...
    @old author   rava (infinityPV ApS)
	@license  BSD (see license.txt)
    This is a INA233 library for arduino 1.9+
...
class INA233{
 public:
  ...
  void begin(void);
  bool begin2(uint8_t addr); //written by zell
  float getBusVoltage_V(void);
  float getShuntVoltage_mV(void);
  float getCurrent_mA(void);
  double CalCurrent_mA(double R_shunt); //written by zell
  ...
  uint16_t RegReadWord(uint8_t reg_address);
  ...
  void RegWriteWord (uint8_t reg_address, uint16_t data);//written by zell
