 ...
 * modified bz zell, 2020.5.18, tested with wrover module, OK!
...
*/
// INIT DISPLAY ------------------------------------------------------------
//initList_new by zell tested works 2020.5.14, can turn on, but random pixels: A0 74; A1 80
static const uint8_t PROGMEM initList_new[] = {
  SSD1351_CMD_COMMANDLOCK, 1, // Set command lock, 1 arg
    0x12,//unlock OLED driver entering CMD 
  SSD1351_CMD_COMMANDLOCK, 1, // Set command lock, 1 arg
    0xB1,//unlock  reg access
  ...
  SSD1351_CMD_DISPLAYOFFSET, 1,
    0x0,
  SSD1351_CMD_STARTLINE, 1, //A1:added by zell, Address increment mode, bit0: 0 horizontal,1 for vertical
    0x0,
  SSD1351_CMD_SETREMAP, 1,//A0:added by zell, Column Address Mapping, flip display ram?
    0x74,//0111 0100: Color sequence is swapped: C 􀃆 B 􀃆 A; Scan from COM[N-1] to COM0. Where N is the Multiplex ratio.Enable COM Split Odd Even, 65K color
	//0x66,//for vertical? 0110 0110: Scan from COM0 to COM[N –1],Column address 127 is mapped to SEG0
  SSD1351_CMD_SETGPIO, 1, //changed val by zell
    0x00,  //0x03:GPIO1 output high!  0x01:HiZ, input disabled
  SSD1351_CMD_FUNCTIONSELECT, 1,
    0x01,  // internal (diode drop)
  SSD1351_CMD_PRECHARGE, 1,
    0x32,
  SSD1351_CMD_SETGRAY, 1, //added by zell
    0x17,
  SSD1351_CMD_VCOMH, 1,
    0x05,
...
static const uint8_t PROGMEM initList[] = {
  ...
  SSD1351_CMD_NORMALDISPLAY, 0,
  SSD1351_CMD_CONTRASTABC, 3,
    0xC8, 0x80, 0xC8,//reset def: 0x8A,0x51,8A
  SSD1351_CMD_CONTRASTMASTER, 1,
    0x0F, //smaller val to reduce seg current!
	//0x08,  //test only
  SSD1351_CMD_SETVSL, 3,
    0xA0, 0xB5, 0x55,
  SSD1351_CMD_PRECHARGE2, 1,
...
void Adafruit_SSD1351::begin_new(uint32_t freq) {

  if(!freq) freq = SPI_DEFAULT_FREQ; // Will move to SPITFT initSPI
  initSPI(freq);

  const uint8_t *addr = (const uint8_t *)initList_new;
  uint8_t        cmd, x, numArgs;
...
void Adafruit_SSD1351::setAddrWindow(
  ...
  sendCommand(SSD1351_CMD_SETCOLUMN); // X range 0x15
  SPI_CS_LOW(); //added by zell
  spiWrite(x1); //bugs no cs low!!!
  spiWrite(x2);
  sendCommand(SSD1351_CMD_SETROW);    // Y range  0x75, Multiple byte command (D/C# = 0 for first byte, D/C# = 1 for other bytes)
  SPI_CS_LOW();
  spiWrite(y1);
  spiWrite(y2);
  sendCommand(SSD1351_CMD_WRITERAM);  // Begin write  0x5c
  
  startWrite(); //added by zell, seems needed! 2020.5.15
}
