...
# V2.4 improve UI, add BtnA test code
...
if OV77XX_EN:
    ...
    while 1:
        try:
                 #sensor.reset(freq=22000000, set_regs=True, dual_buff=False) #OV7740  Loop Time :91ms, run fps:10.98901, fps 12 at serial terminal
                 sensor.reset(freq=22000000, set_regs=True, dual_buff=True) #OV7740  Loop Time :91ms, run fps:10.98901, fps 22 at serial terminal
                 break
        except:
...
sensor_ID = sensor.get_id()  #0x7742 for ov7740  0x77 for ov7725, 0x2642 for OV2640
if (sensor_ID == 30530):
...
while(condition):
    ...
    EXPO_us =  sensor.get_exposure_us()
    run_cnt =run_cnt+1
    if auto_save:
    ...
    #Btn_A to set color bar mode
    if Btn_A.value() == 0 and isButtonPressedA == 0:
        #sensor.set_colorbar(1)
        auto_gain_EN = not auto_gain_EN
        if auto_gain_EN:
            sensor.set_auto_gain(1,8) # given value is for gain ceiling
            sensor.set_auto_exposure(1)
            print("->:Sensor set to auto gain mode!" )
        else:
            sensor.set_auto_gain(0,8) #seems fixed value not working!  ceiling is 10 @ 8 given!
            #sensor.set_auto_exposure(0,100)#works!
            #sde_t sde
            #sensor.set_special_effect(1)
            print("->:Sensor gain set to 8X fixed/ceiling!" )
    ...
    #fps =clock.fps()
    if(run_cnt%15)==0:
       print("gain: "+str(gain)+" Exp. time:"+str(EXPO_us)+"us" )
