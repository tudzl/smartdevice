...
SPIClass hwspi(VSPI); //SCLK = 18, MISO = 19, MOSI = 23, SS = 15
...
void setup(void) {
  ...
  if (HardwareSPI_EN) {
    ...
    hwspi.begin(18, 19, 23, 15); //SCLK, MISO, MOSI, SS，CS, DC, RST
    hwspi.setFrequency(SPI_Freq);
    Serial.println("1.5' SSD1251 RGB OLED init...");
    display.begin_new(SPI_Freq); //adfruit
    Serial.printf("OLED HW SPI mode with freq: %d MHz\r\n", SPI_Freq);
  ...
  //  Serial.println("writePixel red"); //18.18ms, about 2.6 us/pixel
  //  display.writePixel(1, 0, RED); //writePixels
  //  display.writePixel(2, 0, RED); //writePixels
  //  display.writePixel(3, 0, RED); //writePixels
  ...
  while (loop_cnt) {
    ...
    //    display.print("1.5 OLED");//GFX--> write -- not working
    delay(300);
    lcdTestPattern2(); //ok
    delay(300);
    //delay(100);
...
void loop() {
  ...
  //display.writePixel(0, 0, BLACK);
  display.writePixel(run_cnt % 128, run_cnt % 128, GREEN);
  //  display.writePixel(1, 0, RED); //writePixels
  //  display.writePixel(2, 0, RED); //writePixels
  //  display.writePixel(3, 0, RED); //writePixels
  //  display.writePixel(4, 0, RED); //writePixels
