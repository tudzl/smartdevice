...
#define External_Neopixel_NUM_LEDS  64
...
void showSignal()
{
  ...
  for (n = 0; n < SIGNAL_LENGTH / 2; n++)
  {
    ...
    if (n > 0)
    {
      ...
      {
        if (y < VERTICAL_RESOLUTION/2)
        M5.Lcd.drawLine(oldx,    oldy, x,            y, ORANGE );
        else M5.Lcd.drawLine(oldx,    oldy, x,            y, GREEN );
      }
    }
    oldx = x;
    oldy = y;
...
void loop(void)
{
  ...
  for (n = 1; n < SIGNAL_LENGTH; n++)
  {
    ...
    adcBuffer[n] = v - AdcMeanValue;//DC remove
  ...
  int MajorPeak = FFT.MajorPeak(adcBuffer, SIGNAL_LENGTH, SAMPLINGFREQUENCY);
  ...
  int color_offset = MajorPeak/200;
  ...
  for (uint8_t n = 0; n < External_Neopixel_NUM_LEDS; n++)
  {
    ...
    //}
    threshold = 600;
    if (power > threshold)  Ext_neo_pixels.setPixelColor(n, Ext_neo_pixels.Color(55 + n * 4+color_offset, n * 2, 0));
    else
      Ext_neo_pixels.setPixelColor(n, Ext_neo_pixels.Color(0, 0, color_offset));
    threshold *= 1.5;
  }
  Ext_neo_pixels.show();
  ...
  M5.Lcd.print(MajorPeak); M5.Lcd.print(" Hz");
  M5.Lcd.setCursor(205, 21);
  M5.Lcd.print(maxAmplitudeDB); M5.Lcd.print(" dB");
  M5.Lcd.setCursor(205, 41);
  M5.Lcd.print("Mic:"); M5.Lcd.printf("%4.0f", AdcMeanValue);
