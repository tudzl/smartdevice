/*  M5stack fire rainbow pixels ws2812B demo app
     modified by zell
     V 1.2  17.03.2020
     task list, need to save NUM_LEDS to flash and readback
...
void buttonC_wasPressed(void) {
  ...
  //FastLED.addLeds<WS2812B, Neopixel_PIN, GRB>(leds, active_leds).setCorrection(TypicalLEDStrip);
...
void buttonB_wasPressed(void) {
  ...
  if ( Pixel_brightness > 255)
    Pixel_brightness = 10 ;
  Serial.printf("Pixel_brightness: %d\r\n", Pixel_brightness);
...
void loop()
{
  ...
  if ( run_cnt % 100 == 0) {
    ...
    M5.Lcd.printf("Brightness: %d", Pixel_brightness );
...
{
  for (;;) {
    fill_rainbow(leds, NUM_LEDS, gHue, 8);// rainbow effect
