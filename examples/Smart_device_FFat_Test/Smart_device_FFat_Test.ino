...
void testFileIO(fs::FS &fs, const char * path) {
  Serial.printf("--Now testing file I/O with %s ...\r\n", path);
  ...
  size_t i;
  Serial.print("- writing buf[512] to file now:" );
  uint32_t start = millis();
  for (i = 0; i < 2048; i++) {
    if ((i & 0x001F) == 0x001F) {
      //0x1f = 32 dec, total 64 dots
      Serial.print(".");
    }
    file.write(buf, 512);
  }
  ...
  Serial.printf(" - writte speed %.1f bytes/ms\r\n", (float)2048 * 512 / (float)end);
  file.close();
  ...
  if (file && !file.isDirectory()) {
    ...
    size_t flen = len;
    start = millis();
    Serial.print("- reading file to buf[512] now:" );
    while (len) {
      size_t toRead = len;
      if (toRead > 512) {
        toRead = 512;
    ...
    Serial.printf(" - Read speed %.1f bytes/ms\r\n", (float)2048 * 512 / (float)end);
    file.close();
  } else {
    Serial.println("- failed to open file for reading");
  }
