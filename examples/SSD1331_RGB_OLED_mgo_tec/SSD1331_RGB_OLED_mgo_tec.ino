/***************************************************
   2020.3.26 6M SPI frequency is OK!
  This is a example sketch mgo_tec lib demonstrating the graphics
...
//uint32_t SPI_Freq = 6000000;
#define LED_pin 5

ESP32_SSD1331 display = ESP32_SSD1331(SCK, MISO, MOSI, CS, DC, RST, SPI_Freq);
//ESP32_SSD1331 display = ESP32_SSD1331(SCK, MISO, MOSI, CS, DC, RST);
//Esp32Ssd1331Bv2 display = Esp32Ssd1331Bv2(SCK, MISO, MOSI, CS, DC, RST);
// Color definitions 16bit  RGB565
#define  BLACK           0x0000
#define BLUE            0x001F
#define RED             0xF800
#define GREEN           0x07E0
...
//Adafruit_SSD1331 RGB_OLED = Adafruit_SSD1331(&SPI, CS, DC, RST);
...
void setup(void) {
  ...
  //pinMode(CS, OUTPUT); //HSPI SS
  //hspi->beginTransaction(SPISettings(spiClk, MSBFIRST, SPI_MODE0));

  //SPI.begin(SCK, MISO, MOSI, CS);
  Serial.begin(115200);
  Serial.println("hello! MGO_TEC_SSD1331 driver test");
  //display.begin();
  // display.init(); //v2
  display.SSD1331_Init();
  //display.SSD1331_Init(true, 0,0,0); //SPI init, ssd1331 SPI min CLK 150ns---> 7M
  ...
  delay(50);
  //display.Display_Clear(0, 0, 32, 32);
  //display.clearDisplay();
  Serial.println("start screen test now!");
  
  display.Drawing_Pixel_65kColor(32, 32, 31, 0, 0);
  display.Drawing_Pixel_65kColor(33, 32, 0, 63, 0);
  display.Drawing_Pixel_65kColor(34, 32, 0, 0, 31);

  display.Drawing_Pixel_65kColor(32, 40, 31, 0, 31);
  display.Drawing_Pixel_65kColor(34, 40, 0, 63, 31);
  display.Drawing_Pixel_65kColor(36, 40, 31, 63, 0);
  display.Drawing_Pixel_65kColor(38, 40, 31, 61, 31);
  //display.drawPixel65kColor(32,32, 25,5,0);
  delay(1000);

  //RGB_OLED.begin();
  //RGB_OLED.begin2(SPI_Freq,SPI_MODE3,SCK, MISO, MOSI, CS, DC, RST); //adfruits
  //RGB_OLED.begin2(SPI_Freq,SPI_MODE3); //adfruits
  //lcdTestPattern();
  /*
    display.fillScreen(BLACK);
...
void loop() {
  ...
  if(run_cnt > 18000){
    ...
    display.Drawing_Pixel_65kColor(pixel_x, pixel_y, R, G, B);
  }
  else if  (run_cnt > 12000){
    //uint8_t B = rand ();
    display.Drawing_Pixel_65kColor(pixel_x, pixel_y, R, R*2, R);
  }
  else if (run_cnt > 12000)
    display.Drawing_Pixel_65kColor(pixel_x, pixel_y, 0, 0, R);
  else if (run_cnt > 6000)
    display.Drawing_Pixel_65kColor(pixel_x, pixel_y, 0, R, 0);
  else
    display.Drawing_Pixel_65kColor(pixel_x, pixel_y, R, 0, 0);
  run_cnt++;
  if (run_cnt > 30000)
    run_cnt = 0;
  //Serial.printf("run: %d \r\n", run_cnt);
...
/* 
 void lcdTestPattern(void)
  ...
  RGB_OLED.setAddrWindow(0, 0, 96, 64); //include RGB_OLED.startWrite();

  for (h = 0; h < 64; h++) {
    for (w = 0; w < 96; w++) {
      if (w > 83) {
        RGB_OLED.writePixel(w, h, WHITE);
      } else if (w > 71) {
        RGB_OLED.writePixel(w, h, BLUE);
      } else if (w > 59) {
        RGB_OLED.writePixel(w, h, GREEN);
      } else if (w > 47) {
        RGB_OLED.writePixel(w, h, CYAN);
      } else if (w > 35) {
        RGB_OLED.writePixel(w, h, RED);
      } else if (w > 23) {
        RGB_OLED.writePixel(w, h, MAGENTA);
      } else if (w > 11) {
        RGB_OLED.writePixel(w, h, YELLOW);
      } else {
        RGB_OLED.writePixel(w, h, BLACK);
      }
    }
  }
  RGB_OLED.endWrite();
