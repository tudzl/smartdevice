...
void setup() {
  ...
  if (OLED_HWSPI_EN) {
    ...
    display.print("<Firmware V10.2,ling zhou,3.4.2020>");
...
void loop() {
  ...
  if (MAX30205_ok) {
    ...
    if (OLED_HWSPI_EN) {
      display.setTextColor(NEONGREEN, BLACK); //textcolor and bg color
      display.setTextSize(0);
      display.setCursor(1, H_msg0_status );
      display.printf("T:%.2f,", T_max30205);
  ...
  if (M5.BtnA.pressedFor(500) ) {
    buttonA_longPressed();
    Serial.printf("BtnA was long Pressed 500ms\r\n");
    delay(200);
  }
