...
const char* device_ID = "SmartDevice_02";
const char* Combitac_ID_MA = "CT34.0002-P"; //PCB
const char* Combitac_ID_FE = "CT34.0002-S";
...
const char* mqtt_lastwillTopic = "stsmd/CT34.0002-P/info";
...
const char* MQTT_Info_head =  "stsmd/CT34.0002-P/info"; //for new server api
const char* MQTT_data_head =  "stsmd/CT34.0002-P/data";
const char* MQTT_SensorMsg_head = "stsmd/CT34.0002-P/local"; //for old api
const char* MQTT_GlobalMsg_head = "stsmd/CT34.0002-P/global"; //for old api
...
void buttonA_longPressed(void) {
  ...
  if (true == MQTT_active) {
    ...
    delay(50);
    //client.publish("strtd/CT34.0002-P/online", "");
  }
  else {// need improve
    Serial.printf("-->:Smart device MQTT go Online now!\r\n");
    client.publish(MQTT_Info_head, "online");
    delay(50);
    //client.publish("strtd/CT34.0002-P/online", "now online!");
  }
...
void setup() {
  ...
  */
  ...
  client.publish(MQTT_Info_head, "online");
  delay(20);
  //client.publish("strtd/CT34.0002-P/online", "device will go online now"); //old api
...
void loop() {
  ...
  if (run_cnt % 10 == 0) {
    ...
    if (MQTT_active && wifi_ok  ) {
      ...
      //      client.publish("stsmd/CT34.0002-P/light_B", msg);
      //      Serial.printf("MQTT Msg 1 sent: stsmd/CT34.0002-P/light: %.1f Lux\r\n", lux_max44009);

      //      dtostrf(MCP9808_T, 6, 2, msg);
      //      Serial.printf("MQTT raw Msg 2:%s\r\n", msg);
      //      client.publish("stsmd/CT34.0002-P/T_PCB", msg);
      ...
      /*
        ...
        client.publish("strtd/CT34.0002-P/cycle", tmp_string);
      */
      //^^^^^^^^^^^^^^^^^^^^Old APi^^^^^^^^^^^^^^^^^^^^^
