...
/*  Smart device Main PCB code for ESP32 wrover and 6 sensors
    ...
    Version 8.3 MQTT last will active！ need test
    Version 8.2 MQTT wifi config is done！ need improve
...
void setup_wifi() {
  ...
  else if (0 == wifi_config)
  { //read back last time wifi config ssid_pre  bugs!
    readFilestring(FFat, "/Wifi_ssid.txt", ssid_pre);
    //ssid_pre = readFile(FFat, "/Wifi_ssid.txt");;
    //password_pre = readFile(FFat, "/Wifi_password.txt");;
    readFilestring(FFat, "/Wifi_password.txt", password_pre);
    Serial.println(ssid_pre);
    WiFi.begin(ssid_pre, password_pre);
  }
  else
  { Serial.println(ssid4);
    WiFi.begin(ssid4, password4);
...
void setup() {
  ...
  wifi_config = wifi_config_num; // need improve
  ...
  if (MAX30205_sensor.begin2())
  {
    ...
    Serial.println("##: Auto mode, Thyst is 37 degree, TOS is 38 degree!");
    delay(50);
  ...
  char* tmpfilecontent;
   readFilestring(FFat, "/Wifi_config_Nr.txt", tmpfilecontent);
   // atoi()
   wifi_config = unsigned char *tmpfilecontent;
