...
const char* mqtt_lastwillTopic = "stsmd/CT34.0004-P/info";
...
void buttonA_longPressed(void) {
  ...
  if (true == MQTT_active) {
    ...
    delay(50);
    //client.publish("strtd/CT34.0004-P/online", "");
  }
  else {// need improve
    Serial.printf("-->:Smart device MQTT go Online now!\r\n");
    client.publish(MQTT_Info_head, "online");
    delay(50);
    //client.publish("strtd/CT34.0004-P/online", "now online!");
  }
...
void setup() {
  ...
  */
  ...
  client.publish(MQTT_Info_head, "online");
  delay(20);
  //client.publish("strtd/CT34.0004-P/online", "device will go online now"); //old api
  ...
  if (run_cnt % 10 == 0) {
    ...
    if (MQTT_active && wifi_ok  ) {
      ...
      /*
        ...
        client.publish("strtd/CT34.0004-P/cycle", tmp_string);
      */
      //^^^^^^^^^^^^^^^^^^^^Old APi^^^^^^^^^^^^^^^^^^^^^
