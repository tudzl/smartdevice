...
const char* device_ID = "SmartDevice_06";
const char* Combitac_ID_MA = "CT34.0006-P"; //PCB
const char* Combitac_ID_FE = "CT34.0006-S";
...
const char wifi_config_num = 4;
unsigned char wifi_config = 4 ;
...
const char* MQTT_Info_head =  "stsmd/CT34.0006-P/info"; //for new server api
const char* MQTT_data_head =  "stsmd/CT34.0006-P/data";
const char* MQTT_SensorMsg_head = "stsmd/CT34.0006-P/local"; //for old api
const char* MQTT_GlobalMsg_head = "stsmd/CT34.0006-P/global"; //for old api
char MQTT_payload[128] ;
const char  Data_ava_flag = B1001111 ; //ori 1000111

WiFiClient SmartDevice_06;
PubSubClient client(SmartDevice_06); //using name from above wificlient
...
void buttonA_longPressed(void) {
  ...
  if (false == MQTT_active) {
    ...
    delay(50);
    client.publish("stsmd/CT34.0006-P/online", ""); //old strtd
  }
  else {
    Serial.printf("-->:Smart device MQTT go Online now!\r\n");
    client.publish(MQTT_Info_head, "online");
    delay(50);
    client.publish("stsmd/CT34.0006-P/online", "now online!");//old strtd
  }
...
void setup() {
  ...
  Serial.println(F("<<<Firmware Version 6.9, ling zhou, 11.11.2019>>>"));
  ...
  client.publish(MQTT_Info_head, "online");
  delay(100);
  //client.publish("strtd/CT34.0006-P/online", "device will go online now"); //old api
...
void loop() {
  ...
  if (run_cnt % 10 == 0) {
    ...
    if (MQTT_active && wifi_ok  ) {
      ...
      //      client.publish("stsmd/CT34.0006-P/light_B", msg);
      //      Serial.printf("MQTT Msg 1 sent: stsmd/CT34.0006-P/light: %.1f Lux\r\n", lux_max44009);

      //      dtostrf(MCP9808_T, 6, 2, msg);
      //      Serial.printf("MQTT raw Msg 2:%s\r\n", msg);
      //      client.publish("stsmd/CT34.0006-P/T_PCB", msg);
      ...
      /*
        ...
        client.publish("strtd/CT34.0006-P/cycle", tmp_string);
      */
      //^^^^^^^^^^^^^^^^^^^^Old APi^^^^^^^^^^^^^^^^^^^^^
