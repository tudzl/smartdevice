...
/*  Smart device Main PCB code for ESP32 wrover and 6 sensors
    ...
    Version 6.9B WIFI SSID  Final version
...
const float BME280_Hum_offset = 15 ; // calibration with Fluke 971, 2019.12.4 for #9 PCB
...
WiFiClient SmartDevice_09;
PubSubClient client(SmartDevice_09); //using name from above wificlient
...
void buttonA_longPressed(void) {
  ...
  if (false == MQTT_active) {
    ...
    delay(50);
    client.publish("stsmd/CT34.0009-P/online", ""); //old strtd
  }
  else {
    Serial.printf("-->:Smart device MQTT go Online now!\r\n");
    client.publish(MQTT_Info_head, "online");
    delay(50);
    client.publish("stsmd/CT34.0009-P/online", "now online!");//old strtd
  }
...
void setup() {
  ...
  if (digitalRead(BUTTON_A_PIN) == 0) {
    Serial.println("Will Load Wifi config_2, default is config_4");
    wifi_config = 2;
  ...
  Serial.println(F("<<<Firmware Version 6.9B, ling zhou, 5.12.2019>>>"));
