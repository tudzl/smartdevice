...
/*  Smart device Main PCB code for ESP32 wrover and 6 sensors
    ...
    Version 6.3 config Wifi ssid through MQTT 2019.11.11
...
const char* MQTT_SensorMsg_head = "stsmd/SmartDevice_02/local";
const char* MQTT_GlobalMsg_head = "stsmd/SmartDevice_02/global";
char MQTT_payload[100] ;
const char  Data_ava_flag = B1001111 ; //ori 1000111

WiFiClient SmartDevice_02;
PubSubClient client(SmartDevice_02);
...
void buttonA_longPressed(void) {
  ...
  if (false == MQTT_active) {
    ...
    client.publish("strtd/SmartDevice_02/online", "");
  }
  else {
    Serial.printf("Smart device MQTT go Online now!\r\n");
    client.publish("strtd/SmartDevice_02/online", "now online!");
  }
}
...
void setup() {
  ...
  Serial.println(F("<<<Firmware Version 6.3, ling zhou, 11.11.2019>>>"));
  ...
  client.publish("stsmd/SmartDevice_02/info", "device starts now");
  client.publish("strtd/SmartDevice_02/online", "device will go online now");
  // m5mqtt.publish(str('strtd/'+Device_ID+'/online'), "Smart device is online!")
  Serial.println("MQTT sent:strtd/SmartDevice_02/online:device online now");
...
void loop() {
  ...
  if (run_cnt % 10 == 0) {
    ...
    if (MQTT_active  ) {
      ...
      client.publish("stsmd/SmartDevice_02/light", msg);
      Serial.printf("MQTT Msg 1 sent: stsmd/SmartDevice_02/light: %.1f Lux\r\n", lux_max44009);
      ...
      client.publish("strtd/SmartDevice_02/cycle", tmp_string);
      //Serial.printf("MQTT Msg: %s: %s \r\n", MQTT_SensorMsg_head, MQTT_payload);
      //combine msg data
