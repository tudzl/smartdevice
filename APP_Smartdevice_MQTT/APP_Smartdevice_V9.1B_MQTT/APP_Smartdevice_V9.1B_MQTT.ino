...
/*  Smart device Main PCB code for ESP32 wrover and 6 sensors
    ...
    Version 9.1B improve MLX NCIR T filter with mlx_filter ( ZFilter)
    Version 9.0B improve Humidity offset!
...
ZFilter a_filter;    //altitude filte
ZFilter mlx_filter;
...
void loop() {
  ...
  if (BME280_ok) {
    ...
    Serial.printf("BME280 Temp. = %2.2f°C ; Humidity(raw)= %0.2f %% ;pressure= %0.2f hpa\r\n", tmperature, hum, pressure);
  ...
  if (MLX90614_ok) {
    ...
    T_mlx_Kfilter = mlx_filter.Filter(T_MLX_obj);
    Serial.printf("--> MLX90614 Obj T.Zfiltered =  %.2f °C\r\n", T_mlx_Kfilter);
  }
