...
const float BME280_Hum_offset = 15 ; // calibration with Fluke 971, 2019.12.4 for #5 PCB
...
WiFiClient SmartDevice_08;
PubSubClient client(SmartDevice_08); //using name from above wificlient
...
void buttonA_longPressed(void) {
  ...
  if (false == MQTT_active) {
    ...
    delay(50);
    client.publish("stsmd/CT34.0008-P/online", ""); //old strtd
  }
  else {
    Serial.printf("-->:Smart device MQTT go Online now!\r\n");
    client.publish(MQTT_Info_head, "online");
    delay(50);
    client.publish("stsmd/CT34.0008-P/online", "now online!");//old strtd
  }
