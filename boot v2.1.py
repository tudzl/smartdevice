# Boot.py - By: ling - Tue Oct 20 2020 for StickV
# factory default boot.py file modified by ZL

#version 2.1   2021.05.09 improved Boot and QR-code image code, added invert function, tested OK!
...
def boot_menu(cnt):
    ...
    lcd.draw_string(10,100, "Waiting user Btns, A or B?", lcd.ORANGE,lcd.WHITE)
    while (cnt):
        try:
            if but_a.value() == 0:
                print("Btn A pressed!")
...
#2021.5.9: sys_program==1--> "VGA auto save"
#sys_program==2.1--> "face detect";sys_program==2.2--> "QR code"
#added 2020.11.03
try:
    #os.chdir("/flash")
    #os.chdir("/sd")
    ...
    while(True):
        ...
        if but_a.value() == 0 and btnA_status == 1:
            if led_w.value() == 1:
                led_w.value(0)
                GROVE_4.value(1)
                btnA_press_cnt=btnA_press_cnt+1
            else:
                led_w.value(1)
                GROVE_4.value(0)
            btnA_status = 0
