#ifndef _SHQ200MPU9250DEMO_H_
#define _SHQ200MPU9250DEMO_H_
...
class SHQ200MPU9250Demo : public MenuCallBack
{
  MPU9250 IMU;
  SH200Q IMU_SH;
  uint16_t _x;
  int _style;
  int IMU_mode; //0 mpu9250; 1 SH200Q
  float accX = 0.0F;
  float accY = 0.0F;
  float accZ = 0.0F;
  ...
  float gyroZ = 0.0F;
public:
  SHQ200MPU9250Demo() : _x(0), _style(0) {}
  ...
  bool setup()
  {
	...
	if (0 != IMU_SH.Init()) {
      M5.Lcd.println("SH200Q not connected...");
      delay(2000);
      return false;
    }     
	else{
	  M5.Lcd.println("SH200Q detected, init...");
      delay(2000);
	  IMU_mode =1;
	  _x = 0;
	  return true;
  ...
  bool loop()
  {
	...
	if(IMU_mode){
		...
		IMU_SH.getGyroData(&gyroX,&gyroY,&gyroZ);
        IMU_SH.getAccelData(&accX,&accY,&accZ);
		IMU.ax=accX;
		IMU.ay=accY;
		IMU.az=accZ;
		IMU.gx=gyroX;
